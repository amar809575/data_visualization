...
const RelevanceChart = () => {
  ...
  const relevanceData = filteredData.reduce((acc, item) => {
    const year = item.start_year || item.end_year;
    if (year) {
      if (!acc[year]) {
        acc[year] = { relevance: 0, count: 0 };
      }
  ...
  const chartData = [["Year", "Average Relevance"]];
  Object.keys(relevanceData).forEach((year) => {
    const { relevance, count } = relevanceData[year];
    chartData.push([parseInt(year), relevance / count]);
  });

  const options = {
    title: "Yearly Distribution of Relevance",
